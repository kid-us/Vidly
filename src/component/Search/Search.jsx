          ...
          while (randomSearch.size < 18) {
            const data =
              searchQuery.data.results[
                Math.floor(Math.random() * searchQuery.data.results.length)
              ];
  ...
  return (
    <>
      ...
      <div className="container mt-5 pt-5">
        <p className="fs-5 mt-5 fw-semibold">
          Search Resulst for <span className="text-warning">"{query}"</span>
        </p>
            ...
            <div key={index} className="col-lg-2 col-md-4 col-6 my-4">
              <div className="movie-container position-relative">
                ...
                >
                  <img
                    key={search?.title}
                    className="img-fluid rounded vidly-content"
                    src={`${imgUrl}${
                      ...
                      <div
                        className={` ${
                          index === 5 || index === 11 || index === 17
                            ? "movie-modal-left"
                            : "movie-modal-right"
                        } small rounded px-3 py-4 hover-show`}
                      >
                        ...
                        <p className="small">
                          {search.genre_ids.map((id) => (
                            <span className="text-info small me-2" key={id}>
                              {genreName(id)}
                            </span>
                          ))}
                        </p>
                        <p className="small">
                          <span>
                            {search?.release_date || search?.first_search_date}{" "}
                          </span>{" "}
                          &nbsp; &nbsp;
                          <span className="bi-star-fill text-warning">
                            {" "}
                            {search.vote_average}
                          </span>
                        </p>
                        <p className="small text-secondary modal-overview">
                          {search.overview}
                        </p>
                      </div>
                    </div>
              ...
              <p className="item-name small mt-2 text-secondary fw-semibold">
                {search?.name || search?.title}
              </p>
            </div>
          ))}
